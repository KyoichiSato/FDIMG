                      ...
                      unsigned char mode) /* モード MODE2D/MODE2DD */
{
  ...
  if (disksense(FD2DD | 0, &ret))
    if ((0x04 & ret) == 0)
      val &= ~(1 << 0); /* DRIVE0 */
  if (disksense(FD2DD | 1, &ret))
    if ((0x04 & ret) == 0)
      val &= ~(1 << 1); /* DRIVE1 */
  if (disksense(FD2DD | 2, &ret))
    if ((0x04 & ret) == 0)
      val &= ~(1 << 2); /* DRIVE2 */
  if (disksense(FD2DD | 3, &ret))
    if ((0x04 & ret) == 0)
      val &= ~(1 << 3); /* DRIVE3 */

  if (MODE2D == mode)
    val &= ~(1 << device); /* 2D  ビットクリア */
  else
    val |= (1 << device); /* 2DD ビットセット */
...
戻り値 0:異常終了 0以外:正常終了 */
unsigned char seek(unsigned char device, unsigned char C) /* シリンダ番号 */
{
  union REGS reg;
...
戻り値 0:異常終了 0以外:正常終了 */
unsigned char readid(unsigned char device, unsigned char MF, /* MFM/FM */
...
unsigned char readdata(unsigned char device, unsigned char MF, /* MFM / FM */
...
戻り値はエラーコード！ 0:正常終了 ！変更しました！ */
unsigned char readdeleted(unsigned char device, unsigned char MF, /* MFM / FM */
                          unsigned char C,                        /* シリンダ番号 */
                          unsigned char H,                        /* ヘッド番号 */
                          unsigned char R,                        /* セクタ番号 */
                          unsigned char N,                        /* セクタ長コード */
...
戻り値 0:異常終了 0以外:正常終了 */
unsigned char verify(unsigned char device, unsigned char MF, /* MFM / FM */
                     unsigned char C,                        /* シリンダ番号 */
                     unsigned char H,                        /* ヘッド番号 */
                     unsigned char R,                        /* セクタ番号 */
                     unsigned char N,                        /* セクタ長コード */
...
戻り値 0:異常終了 0以外:正常終了 */
unsigned char writedata(unsigned char device, unsigned char MF, /* MFM / FM */
                        unsigned char C,                        /* シリンダ番号 */
                        unsigned char H,                        /* ヘッド番号 */
                        unsigned char R,                        /* セクタ番号 */
                        unsigned char N,                        /* セクタ長コード */
...
戻り値 0:異常終了 0以外:正常終了 */
unsigned char writedeleted(unsigned char device, unsigned char MF, /* MFM / FM */
                           unsigned char C,                        /* シリンダ番号 */
                           unsigned char H,                        /* ヘッド番号 */
                           unsigned char R,                        /* セクタ番号 */
                           unsigned char N,                        /* セクタ長コード */
...
ギャップ長はBIOSが決めてくれる */
unsigned char trackfmt(unsigned char device, unsigned char MF, /* MFM / FM */
                       unsigned char C,                        /* シリンダ番号 */
                       unsigned char H,                        /* ヘッド番号 */
                       unsigned char N,                        /* セクタ長コード */
...
*/
unsigned char readdiag(unsigned char device, unsigned char MF, /* MFM / FM */
                       unsigned char H,                        /* ヘッド番号 */
                       unsigned char N,                        /* セクタ長コード */
                       unsigned char *buff)                    /* 出力を書き込むアドレス */
{
...
unsigned char fdcheck(void)
{
  ...
  /*DISKBIOSで両面/片面の切り替えを行ってみて、設定できているかを確認する*/

  reg.h.ah = 0x0E; /* 全ドライブ片面モードにする */
  reg.h.al = 0x10;
  int86(DISKBIOS, &reg, &reg);
  if (0 != reg.x.cflag)
    return FALSE;

  if (!disksense(0, &status))
    return FALSE;
  status &= 0x01; /*bit0のみにする*/
  if (0 != status)
    return FALSE; /*片面じゃない*/
  ...
  if (0 == status)
    return FALSE; /*両面じゃない*/
#endif
  /*PC98のFDモードレジスタが読み書きできるかを確認する*/
#define FDCMODER 0x00be
/*0x00beはOADGリファレンスマニュアルのI/Oポート一覧では未使用の様子*/
/* W 0b00000***
            ||+- PORTEXC I/Oポートアドレスの設定 0:640KB 1:1MB
            |+-- FDD EXC FDDのモード設定 0:640KB 1:1MB
            +--- EMTON 0:常にモーターオン 1:MTONビットでモーター制御
   R 0bxxxx****
           |||+- PORTEXC
           ||+-- FDD EXC
           |+--- FIX 0:I/OポートアドレスPORTEXCビットで設定 1:I/Oポートアドレス固定
           +---- DSW ディップスイッチの状態 0:640KB 1:1MB */
#define fdc_MODE(data) outp(FDCMODER, data)
#define fdc_MODEread() inp(FDCMODER)
  unsigned char temp;

  temp = fdc_MODEread(); /*元の値を保存しておく*/
  temp &= 0x03;          /*下位2ビットだけにする bit2は0 常にモーターオン*/

  /*下位2ビットに書き込みできるかを確認する*/
  fdc_MODE(0x03);
  if (0x03 != (0x03 & fdc_MODEread()))
  {
    fdc_MODE(temp); /*元に戻す*/
    return FALSE;
  }
  fdc_MODE(0x00);
  if (0x00 != (0x03 & fdc_MODEread()))
  {
    fdc_MODE(temp); /*元に戻す*/
    return FALSE;
  }
